...
function mapTypeString(typeString: string): string {
  ...
  // Handle Map<K, V> -> @collection.JsMap[K, V]
  const mapMatch = typeString.match(/^Map<(.+),\s*(.+)>$/);
  if (mapMatch) {
    return `@collection.JsMap[${mapTypeString(mapMatch[1])}, ${mapTypeString(mapMatch[2])}]`;
  }

  // Handle Set<T> -> @collection.JsSet[T]
  const setMatch = typeString.match(/^Set<(.+)>$/);
  if (setMatch) {
    return `@collection.JsSet[${mapTypeString(setMatch[1])}]`;
  }
...
`;
      ...
      // Extract method name and convert to camelCase
      const methodName = func.name.split("::")[1] || func.name;
...
/**
 * Generate .mbti (MoonBit interface) file from MbtBinding
 *
 * .mbti files are interface declarations without implementation,
 * generated by `moon info` for type information sharing.
 *
 * @param binding - The MbtBinding object from parseDts()
 * @returns Generated .mbti content
 *
 * @example
 * ```ts
 * const binding = parseDts(dtsContent, "example.d.ts");
 * const mbti = generateMbti(binding);
 * // => "package \"...\"\npub struct ...\nfn ..."
 * ```
 */
export function generateMbti(binding: MbtBinding): string {
  const lines: string[] = [];

  // Header comment
  lines.push("// Generated from .d.ts by mbts");
  ...
  // Types section
  if (binding.externTypes.length > 0 || binding.types.length > 0) {
    lines.push("// Types and methods");

    // Extern types (external type declarations)
    binding.externTypes.forEach((t) => {
      // Convert "type TypeName" or "type TypeName[T]" format
      lines.push(`pub ${t}`);
    });

    // Struct and enum definitions
    binding.types.forEach((t) => {
      lines.push(generateMbtiType(t));
    });
  ...
  if (binding.functions.length > 0) {
    binding.functions.forEach((f) => {
      lines.push(generateMbtiFunction(f));
    });
    lines.push("");
  }
...
/**
 * Generate .mbti type definition (struct, enum, type)
 */
function generateMbtiType(type: MbtType): string {
  const typeParams =
    type.typeParams && type.typeParams.length > 0
      ? `[${type.typeParams.join(", ")}]`
      : "";
...
/**
 * Generate .mbti function signature (without implementation)
 */
function generateMbtiFunction(func: MbtFunction): string {
  const typeParams =
    func.typeParams && func.typeParams.length > 0
      ? `[${func.typeParams.join(", ")}]`
      : "";

  // For .mbti, parameters are type-only: (Type, Type) -> ReturnType
  const paramTypes = func.params.map((p) => p.type).join(", ");

  // Handle async return type
  const returnType =
    func.isAsync && !func.returnType.startsWith("Promise[")
      ? `Promise[${func.returnType}]`
      : func.returnType;

  // .mbti format: fn Name(ParamTypes) -> ReturnType
  // or for methods: fn Type::method(ParamTypes) -> ReturnType
  return `pub fn${typeParams} ${func.name}(${paramTypes}) -> ${returnType}`;
}

/**
 * Convert a .d.ts file and generate .mbt, .mbti, and optionally glue .js
 */
export function dtsToMbtWithMbti(
  content: string,
  filename: string,
  options: ConvertOptions = {}
): { mbt: string; mbti: string } {
  const binding = parseDts(content, filename, options);
  return {
    mbt: generateMbt(binding),
    mbti: generateMbti(binding),
  };
}
